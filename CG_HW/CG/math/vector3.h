...
class vector3
{
	...
	// calc 0 to 2pi angle assuming z=0 (plane)
	double angle2d(vector3 const& b) const;
	// calc -pi to pi angle assuming z=0 (plane)
	double angle2ds(vector3 const& b) const;

	// calc -pi to pi angle assuming axis=0 (plane): axis 0:x, 1: y, 2:z
	...
	friend std::ostream& operator<<(std::ostream& os, const vector3& arg);
};
