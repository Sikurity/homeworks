...
void B_Modified(float x, float *fn, float *df)
{
	*fn = x - exp(-x);
	*df = 1 + exp(-x);
}
...
void C_Modified(float x, float *fn, float *df)
{
	*fn = sign(x) * (cos(x) + x * x / 2.0f - 1.0f);
	*df = sign(x) * (x - sin(x));
}
...
int main()
{
	...
	printf("TRUE VALUE = %f\n", 0.56751098f);
	printf("X when f(x) = 0 : %f\n\n", rtsafe(B_Modified, B0, B1, XACC));

	printf("3. cos(x + sqrt(2)) + x ( x / 2 + sqrt(2)) = 0, on [-2, -1]\n");
	printf("TRUE VALUE = %f\n", -1.41421356f);
	printf("X when f(x) = 0 : %f\n\n", rtsafe(C_Modified, C0 + sqrt(2), C1 + sqrt(2), XACC) - sqrt(2));
